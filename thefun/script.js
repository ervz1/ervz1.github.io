...
function langchange(el=null) {
    ...
    windowList.forEach(element => {
        if (!winContents[element.id].content.nolang) {
...
function newWindow(id, title, icon, content, properties={}) {
    ...
    if (!properties.pos) {
        properties.pos = {x:150 + 25*windowList.length, y:100+25*windowList.length}
    }
...
function dragWindow(event, id) {
    ...
    document.onmousemove = (e) => {
        ...
        if (e.clientY < 50 && !contentWindow.dataset.nomaximize) {
            ...
            maxeff.style.height = window.innerHeight
        } else {
            maxeff.style.left = 0
            maxeff.style.top = 0
            maxeff.style.width = "100%"
    ...
    document.onmousemove = (e) => {
        ...
        switch (xDir) {
            case -1:
                ...
                if (newWidth >= 200 && newWidth <= window.innerWidth && e.clientX < window.innerWidth && e.clientX > 0) {
        ...
        switch (yDir) {
            case -1:
                ...
                if (newHeight >= 150 && newHeight <= window.innerHeight && e.clientY < window.innerHeight && e.clientY > 0) {
            ...
            case 1:
                newHeight = winSize.y - (event.clientY - e.clientY)
                if (newHeight >= 150 && newHeight <= window.innerHeight && e.clientY < (window.innerHeight-55) && e.clientY > 0) {
                    contentWindow.style.height = `${newHeight}px`
                }
                break
        }
...
function minimize(id, onclose = false) {
    ...
    let rect = contentWindow.getBoundingClientRect()
    contentWindow.querySelector(".maxeffect").style.left = rect.width/2
    contentWindow.querySelector(".maxeffect").style.top = rect.height/2
...
function changeWindowRes(id, ignorePos = false) {
    ...
    if (contWin.getAttribute("data-max") == "true") {
        ...
        contWin.style.width = res[0]
        contWin.style.height = res[1]
        if (!ignorePos) {
            contWin.style.left = pos[0]
            contWin.style.top = pos[1]
        }
    } else {
